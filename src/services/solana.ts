...
export class SolanaService {
    ...
    try {
      ...
      if (idlModule.PLANET_NFT_IDL) {
        this.program = new Program(
          idlModule.PLANET_NFT_IDL as any,
          PROGRAM_ID,
          provider
        );
  ...
  private verifyIDL(program: Program): void {
    ...
    if (!mintInstruction) {
      console.warn(
        "⚠️ IDL verification: mint_planet_nft instruction not found"
      );
      return;
    }

    const mintAccount = mintInstruction.accounts?.find(
      (acc: any) => acc.name === "mint"
    );
    const mintAuthorityAccount = mintInstruction.accounts?.find(
      (acc: any) => acc.name === "mint_authority"
    );

    console.log("🔍 IDL Verification:");
    console.log("  Mint PDA seeds:", (mintAccount as any)?.pda?.seeds);
    console.log(
      "  Mint Authority PDA seeds:",
      (mintAuthorityAccount as any)?.pda?.seeds
    );

    // Check if mint_authority has correct seeds
    const mintAuthoritySeeds = (mintAuthorityAccount as any)?.pda?.seeds;
    if (mintAuthoritySeeds) {
      ...
      if (hasCorrectSeeds) {
        ...
        console.warn(
          '⚠️ IDL Verification: mint_authority seeds may be incorrect. Expected ["mint_authority", planet_id]'
        );
      }
    }
  }
  ...
  async mintPlanetNFT(
    ...
    try {
      ...
      if (mintPda.toBase58() === mintAuthorityPda.toBase58()) {
        console.warn(
          "⚠️ WARNING: mint and mint_authority PDAs are the same! This suggests IDL is incorrect."
        );
      } else {
        console.log(
          "✅ Verified: mint and mint_authority PDAs are different (correct!)"
        );
      }
      ...
      if (program.idl && program.idl.instructions) {
        ...
        if (mintInstruction) {
          ...
          console.log(
            "📋 Instruction args details:",
            mintInstruction.args?.map((arg: any) => ({
              name: arg.name,
              type: arg.type,
            }))
          );
          ...
          const accountDetails = mintInstruction.accounts.map((acc: any) => {
            ...
            if ((acc as any).pda) {
              details.pda = {
                seeds: (acc as any).pda.seeds,
                programId: (acc as any).pda.programId,
              };
            }
            return details;
          });
          ...
          const pdaAccounts = mintInstruction.accounts.filter(
            (acc: any) => (acc as any).pda
          );
          console.log(
            "🔑 PDA accounts:",
            pdaAccounts.map((acc: any) => {
              const seedDetails = (acc as any).pda?.seeds?.map((seed: any) => {
                const detail: any = {
                  kind: seed.kind,
                  raw: seed,
                };
        ...
        if (mintInstruction && mintInstruction.accounts) {
          ...
          mintInstruction.accounts.forEach((acc: any) => {
            ...
            if ((acc as any).pda) {
              ...
              console.log(
                `📌 PDA account ${accName} will be provided manually`
              );
            ...
            if (accountVariations[accName]) {
              ...
              accountsObj[accName] =
                accountVariations[accName.replace(/_/g, "")];
            } else {
              // Log missing account
              console.warn(`⚠️ Missing account mapping for: ${accName}`, {
                hasPDA: !!(acc as any).pda,
                pdaSeeds: (acc as any).pda?.seeds,
              });
            }
          });
        }
      ...
      console.log("📦 Instruction args being sent:", {
        planetId,
        planetName,
        metadataUri,
      });
      ...
      try {
        ...
        console.warn(
          "⚠️ .rpc() failed, trying simplified approach:",
          rpcError.message
        );
        ...
        try {
          ...
          console.log(
            "💡 Note: For full NFT minting, the Anchor program needs to be properly configured."
          );
          console.log(
            "💡 This demo shows wallet connection and transaction signing works!"
          );
